...
$customURL =  \Yii::$app->getUrlManager()->createUrl('forms/fetch-detailed-twig-theme');
$saveURL =  \Yii::$app->getUrlManager()->createUrl('forms/save-detailed-twig-theme');

$this->registerJs( <<< EOT_JS

   $("#theme").change(function(){
        var new_theme=$("#theme").val();
        location.href='?r=forms/fdetails&reload=true&theme=' + new_theme;
  ...
  $('.twig_event').click(function (element) {  
      ...
      $.get(
                '{$customURL}',{renderer:id,theme_id:theme,id:twig_id},
                function(data) { 
...
<table id="datalisting" class="table table-bordered table-striped">
    ...
    <tbody>
      ...
      foreach ($records as $user) :
          $filename="";    
      if(($user['hasContents']!=null) && ($user['hasContents']['theme_id']==$selected_theme) && ($user['hasContents']['code']!="" && $user['hasContents']['renderer_type']=="D")):
          if(substr($user['hasContents']['filename'], 0, strlen("twig/")) === "twig/"):
                $filename = $user['hasContents']['filename'];
          endif;
      ?>
        <tr><td><font color='green'><?=$user['title']?></font></td><td><font color='green'><?=$user['title']?></font></td></td><td> <a href='#' class='twig_event' id='<?=$user['form_id']?>' page_id="<?=$user["id"]?>" event_name='<?=$user['title']?>' twig_id='<?=$user['hasContents']['id']?>' title="User Create Twig" filename="<?=$filename?>"><small><i class="fa fa-file-code-o"></i></small></a> </td>
        <?php
            else:
        ?>
        <tr><td><?=$user['title']?></td><td><?=$user['title']?></td></td><td> <a href='#' class='twig_event' id='<?=$user['form_id']?>' event_name='<?=$user['title']?>' page_id="<?=$user["id"]?>" twig_id='<?=$user['hasContents']['id']?>' title="User Create Twig" filename="<?=$filename?>"><small><i class="fa fa-file-code-o"></i></small></a> </td>
     <?php
        endif;
      endforeach;
     ?>
