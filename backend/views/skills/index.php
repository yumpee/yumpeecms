...
$saveURL = \Yii::$app->getUrlManager()->createUrl('skills/save');
$deleteURL = \Yii::$app->getUrlManager()->createUrl('skills/delete');
...
<div id="addCategories" {{^edit}}class="collapse"{{/edit}}>
    ...
    <table class="table">
        <tr><td>Skill Name<td><input name="name" id="name" value="<?=$name?>" class="form-control" type="text" />
...
<table id="datalisting" class="table table-bordered table-striped">
    ...
    <tbody>
      ...
      ?>
        <tr><td><?=$user['name']?></td><td><td><a href='?actions=edit&id=<?=$user['id']?>&r=skills/extensions'><small><i class="glyphicon glyphicon-user"></i></small></a> <a href='?actions=edit&id=<?=$user['id']?>&r=skills/index'><small><i class="glyphicon glyphicon-pencil"></i></small></a> <a href='#' class='delete_event' id='<?=$user['id']?>' event_name='<?=$user['name']?>'><small><i class="glyphicon glyphicon-trash"></i></small></a>  </td>
     <?php
      endforeach;
     ?>
    </tbody>
